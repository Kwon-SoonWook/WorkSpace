...
public class BookManagement extends Frame implements ActionListener {
	...
	// 책 정보 등록
	Label lb_book_add_title, lb_book_add_bname, lb_book_add_genre, lb_book_add_author, lb_book_add_publisher,
			lb_book_add_msg;
	...
	public String userList() throws Exception {
		...
		while (rs.next()) {
			str = str + rs.getInt("person_id") + "\t   "
					+ String.format("%-" + (15 - (rs.getString("person_name").length() * 2)) + "s",
							rs.getString("person_name"))
					+ String.format("%-" + (15 - (rs.getString("addr").length() * 2)) + "s", rs.getString("addr"))
					+ rs.getString("tel") + "\t" + rs.getString("birth") + "\n";
		}
		return str;
	}
	...
	public void userAdd() throws Exception {
		...
		if (tf_user_add_name.getText().equals("") || tf_user_add_tel.getText().equals("")
				|| tf_user_add_addr.getText().equals("") || tf_user_add_birth.getText().equals("")) {
	...
	public void bookAdd() throws Exception {
		...
		if (tf_book_add_bname.getText().equals("") || tf_book_add_author.getText().equals("")
				|| tf_book_add_publisher.getText().equals("")) {
			lb_book_add_msg.setText("입력되지 않은 정보가 있습니다. 입력 정보를 확인해주세요.");
		} else {
			sql = "insert into book (book_id, book_name, author, publisher) " + "values ("
					+ genreId.get(tf_book_add_genre.getText()) + " ,?,?,?)";
			ps = con.prepareStatement(sql);
	...
	public void searchInfo() throws Exception {
		...
		if (cb_book.getState()) {
			ps = con.prepareStatement("SELECT book_id, book_name, author, publisher, genre, " + "    CASE "
					+ "        WHEN book_id IN (SELECT book_id FROM records) THEN '대여 불가' " + "        ELSE '대여 가능' "
					+ "    END AS status " + "FROM book, genres " + "WHERE book_id BETWEEN min AND max "
					+ "    AND book_name LIKE ? " + "ORDER BY book_id ASC");
			ps.setString(1, "%" + key + "%");
			rs = ps.executeQuery();
			...
			while (rs.next()) {
				String row = rs.getInt("book_id") + "\t"
						+ String.format(
								"%-" + (48 - (rs.getString("book_name").length() * 2)) + "s", rs.getString("book_name"))
						+ rs.getString("author") + "\t"
						+ String.format("%-" + (20 - (rs.getString("publisher").length() * 2)) + "s",
			...
			ps = con.prepareStatement(
					"SELECT person.person_id, person_name, tel, addr, birth, NVL(5 - lend_per_person, 5) AS lend_limit "
							+ "FROM person, (SELECT person_id, COUNT(person_id) AS lend_per_person "
							+ "              FROM records " + "              GROUP BY person_id) lend_per_person "
							+ "WHERE person.person_id = lend_per_person.person_id(+) " + "AND person_name LIKE ?");
			ps.setString(1, "%" + key + "%");
			rs = ps.executeQuery();
			...
			while (rs.next()) {
				String row = rs.getInt("person_id") + "\t "
						+ String.format("%-" + (15 - (rs.getString("person_name").length() * 2)) + "s",
								rs.getString("person_name"))
						+ rs.getString("tel") + "\t   "
						+ String.format("%-" + (15 - (rs.getString("addr").length() * 2)) + "s", rs.getString("addr"))
						+ rs.getString("birth") + "\t" + rs.getInt("lend_limit") + "권 (" + (5 - rs.getInt("lend_limit"))
						+ "권 대여중)\n";
				ta_show_result.append(row);
			}
		}
